...
/// 91爬虫 - 使用headless_chrome提取列表，reqwest爬取详情
#[derive(Clone)]
pub struct NineSpider {
    website: Website,
    client: Client,
}
...
impl NineSpider {
    ...
    async fn fetch_m3u8_from_detail(&self, video_id: &str) -> Option<String> {
        let video_url = self.build_url(&format!("/archives/{}", video_id));
    ...
    fn extract_title(&self, html: &str) -> String {
        let title_pattern = Regex::new(
            r#"(?is)<h1\b[^>]*class\s*=\s*['\"][^'\"]*\bpost-title\b[^'\"]*['\"][^>]*>(.*?)</h1>"#,
        )
        .unwrap();
        let fallback_pattern = Regex::new(
            r#"(?is)<h1\b[^>]*itemprop\s*=\s*['\"][^'\"]*\bheadline\b[^'\"]*['\"][^>]*>(.*?)</h1>"#,
        )
        .unwrap();

        if let Some(cap) = title_pattern
            .captures(html)
            .or_else(|| fallback_pattern.captures(html))
        {
            ...
            for item in &video_items {
                if let Some((_, m3u8_url)) = results.iter().find(|(id, _)| id == &item.video_id) {
                    if let Some(ref url) = m3u8_url {
                        // 获取标题
                        let video_url = spider.build_url(&format!("/archives/{}", item.video_id));
